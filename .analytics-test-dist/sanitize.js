...
exports.normalizeElementType = normalizeElementType;
...
function sanitizeAnalyticsProps(input) {
    const sanitized = {};
    for (const [rawKey, rawValue] of Object.entries(input)) {
        if (isForbiddenKey(rawKey))
            continue;
        const key = rawKey;
        ...
        if (key === 'target_platform') {
            const platform = normalizePlatform(rawValue);
            if (platform)
                sanitized.target_platform = platform;
            continue;
        }
        if (key === 'service') {
            const platform = normalizePlatform(rawValue);
            if (platform)
                sanitized.service = platform;
            continue;
        }
        if (key === 'element_type') {
            const elementType = normalizeElementType(rawValue);
            if (elementType)
                sanitized.element_type = elementType;
            continue;
        }
        if (key === 'element_type_guess') {
            const elementType = normalizeElementType(rawValue);
            if (elementType)
                sanitized.element_type_guess = elementType;
            continue;
        }
